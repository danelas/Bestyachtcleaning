...
form?.addEventListener('submit', async (e) => {
  ...
  if (!data.zip || data.zip.trim().length < 3) { form.zip.focus(); toast('Please enter your marina name or ZIP.'); return; }
  if (!data.vessel || data.vessel.trim().length < 3) { form.vessel.focus(); toast('Tell us about your vessel (length + make).'); return; }
  if (!data.service) { form.service.focus(); toast('Pick a service so we can match you with the right crew.'); return; }
  ...
  btn.disabled = false;
  btn.textContent = '✓ Sent — a captain will reply within the hour';
  toast(`Thanks ${data.fname}! A marine detailer will reach out within the hour.`, 5000);
